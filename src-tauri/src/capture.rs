...
pub struct CaptureController { running: Arc<AtomicBool>, pub packets: Arc<Mutex<u64>>, pub dropped: Arc<Mutex<u64>> }
...
unsafe impl Send for CaptureController {}
unsafe impl Sync for CaptureController {}

fn load() -> Result<(Library, PcapFns), String> {
    #[cfg(target_os = "windows")]
    {
        let lib = Library::new("wpcap.dll").map_err(|e| format!("Npcap/wpcap.dll not available: {e}"))?;
        let f = (
...
pub fn list_interfaces() -> Result<Vec<CaptureInterface>, String> {
    let (_lib, (find, free, _, _, _)) = load()?;
    let mut list = std::ptr::null_mut();
    let mut err = [0i8; 256];
    let rc = unsafe { find(&mut list, err.as_mut_ptr()) };
    if rc != 0 { return Err(cstr(err.as_ptr())); }
    let mut out = Vec::new();
    let mut cur = list;
    while !cur.is_null() {
        unsafe {
...
pub fn start(app: AppHandle, controller: Arc<CaptureController>, interface: Option<String>) -> Result<(), String> {
    ...
    let interface = interface.or_else(|| list_interfaces().ok().and_then(|v| v.into_iter().find(|i| !i.name.is_empty()).map(|i| i.name))).ok_or("No Npcap capture interface found")?;
...
fn capture_loop(app: &AppHandle, interface: &str, running: Arc<AtomicBool>, packets: Arc<Mutex<u64>>, dropped: Arc<Mutex<u64>>) -> Result<(), String> {
    let (_lib, (_, _, open, next, close)) = load()?;
    let name = CString::new(interface).map_err(|_| "Invalid capture interface name".to_string())?;
    let mut err = [0i8; 256];
    let handle = unsafe { open(name.as_ptr(), 65535, 1, 250, err.as_mut_ptr()) };
    ...
    while running.load(Ordering::Relaxed) {
        ...
        if rc == 1 && !hdr.is_null() && !data.is_null() {
            ...
            if let Some(event) = parse_packet(bytes, h.len) {
...
fn parse_packet(b: &[u8], size: u32) -> Option<PacketEvent> {
    ...
    let ihl = ((b[14] & 0x0f) as usize) * 4;
    if ihl < 20 || b.len() < 14 + ihl { return None; }
    let src = format!("{}.{}.{}.{}", b[26], b[27], b[28], b[29]);
    let dst = format!("{}.{}.{}.{}", b[30], b[31], b[32], b[33]);
    let proto = b[23];
    let mut protocol = match proto { 6 => "TCP", 17 => "UDP", 1 => "ICMP", _ => "IP" }.to_string();
    ...
    if (proto == 6 || proto == 17) && b.len() >= l4 + 4 {
        ...
        let port = if [80,443,53,22,25,110,143,993,995].contains(&dp) { dp } else { sp };
        service = match port { 53 => "DNS", 80 => "HTTP", 443 => "HTTPS", 22 => "SSH", 25 => "SMTP", 993 => "IMAPS", 995 => "POP3S", _ => "Unknown" }.into();
        confidence = if service != "Unknown" { 60 } else { 0 };
        protocol = if proto == 6 { "TCP" } else { "UDP" }.into();
        if port == 53 && proto == 17 { if let Some(domain) = dns_name(&b[l4+8..]) { service = service_from_domain(&domain); confidence = 98; } }
        if port == 443 && proto == 6 { if let Some(domain) = tls_sni(&b[l4..]) { service = service_from_domain(&domain); confidence = 94; } }
    }
    Some(PacketEvent { timestamp: chrono_like_now(), source: src, destination: dst, protocol, size, service, confidence })
}

fn service_from_domain(domain: &str) -> String {
    let d = domain.to_ascii_lowercase();
    let table = [("youtube.com","YouTube"),("googlevideo.com","YouTube"),("ytimg.com","YouTube"),("google.com","Google"),("gstatic.com","Google"),("github.com","GitHub"),("githubusercontent.com","GitHub"),("discord.com","Discord"),("discord.gg","Discord"),("instagram.com","Instagram"),("facebook.com","Facebook"),("microsoft.com","Microsoft"),("live.com","Microsoft"),("steampowered.com","Steam"),("steamcontent.com","Steam"),("telegram.org","Telegram"),("t.me","Telegram")];
    table.iter().find(|(suffix,_)| d == *suffix || d.ends_with(&format!(".{suffix}"))).map(|(_,name)| (*name).into()).unwrap_or_else(|| domain.into())
}

fn dns_name(data: &[u8]) -> Option<String> {
    if data.len() < 13 { return None; }
    let mut pos = 12usize; let mut labels = Vec::new();
    for _ in 0..32 { let n=*data.get(pos)? as usize; pos+=1; if n==0 { break; } if n&0xc0 != 0 || n>63 || pos+n>data.len(){return None;} labels.push(std::str::from_utf8(&data[pos..pos+n]).ok()?.to_string()); pos+=n; }
    if labels.is_empty(){None}else{Some(labels.join("."))}
}

fn tls_sni(data: &[u8]) -> Option<String> {
    if data.len() < 6 || data[0] != 0x16 || data[5] != 0x01 { return None; }
    let needle=b"\x00\x00"; let start=6; let end=data.len().min(4096);
    let mut i=start; while i+4<end { if &data[i..i+2]==needle { let n=u16::from_be_bytes([data[i+2],data[i+3]]) as usize; if n>0 && n<256 && i+4+n<=end { if let Ok(s)=std::str::from_utf8(&data[i+4..i+4+n]) { if s.contains('.') && s.chars().all(|c| c.is_ascii_graphic()||c=='.'||c=='-'){return Some(s.to_string());} } } } i+=1; } None
}
...
fn chrono_like_now()->String{ use std::time::{SystemTime,UNIX_EPOCH}; let ms=SystemTime::now().duration_since(UNIX_EPOCH).map(|d|d.as_millis()).unwrap_or(0); format!("{}",ms) }
