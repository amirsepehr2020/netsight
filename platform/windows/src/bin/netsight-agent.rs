//! Localhost dashboard bridge: real Npcap capture -> core correlation -> JSON snapshot.
//! Intended for authorized network observability on the host running NetSight.
        ...
        match event {
            CaptureEvent::Packet(packet) => {
                if let Ok(normalized) = normalize_ethernet(packet.timestamp_micros, packet.captured_len, packet.original_len, &packet.data) {
                    let Some(source) = normalized.source_ip else { continue };
                    let Some(destination) = normalized.destination_ip else { continue };
                    ...
                    let protocol = match normalized.transport { Some(netsight_core::packet::TransportProtocol::Tcp) => "TCP", Some(netsight_core::packet::TransportProtocol::Udp) => "UDP", Some(netsight_core::packet::TransportProtocol::Other(n)) => { Box::leak(format!("IP/{n}").into_boxed_str()) }, None => "IP" };
                    let key = FlowKey { source, destination, protocol: protocol.to_string(), source_port: normalized.source_port, destination_port: normalized.destination_port };
                    traffic.record(key, normalized.captured_len as u64);
...
fn handle_http(mut stream: TcpStream, shared: Shared) -> Result<()> {
    ...
    let (status, content_type, body) = match path {
        ...
        "/api/health" => ("200 OK", "application/json", "{\"ok\":true,\"service\":\"netsight-agent\"}".into()),
