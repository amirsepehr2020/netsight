...
#[derive(Clone)]
struct Shared { snapshot: Arc<Mutex<String>> }
        ...
        match event {
            CaptureEvent::Packet(packet) => {
                if let Ok(normalized) = normalize_ethernet(packet.timestamp_micros, packet.captured_len, packet.original_len, &packet.data) {
                    ...
                    let key = FlowKey { source, destination, protocol: protocol.clone(), source_port: normalized.source_port, destination_port: normalized.destination_port };
...
fn start_http_server(shared: Shared) -> Result<()> {
    ...
    thread::Builder::new().name("netsight-dashboard-http".into()).spawn(move || {
        for stream in listener.incoming().flatten() { let state = shared.clone(); let _ = thread::spawn(move || { let _ = handle_http(stream, state); }); }
