...
impl Default for CaptureConfig { fn default()->Self{Self{snaplen:65_535,read_timeout_ms:250,promiscuous:true,immediate_mode:true,queue_capacity:4096}} }
...
#[cfg(windows)]
pub fn list_capture_devices()->Result<Vec<CaptureDevice>>{Ok(Device::list().context("failed to enumerate Npcap capture devices")?.into_iter().map(|d|CaptureDevice{name:d.name,description:d.desc}).collect())}
#[cfg(not(windows))]
