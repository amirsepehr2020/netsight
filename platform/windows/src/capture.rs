...
pub struct CaptureMetrics { pub packets_received: u64, pub packets_dropped: u64, pub bytes_received: u64, pub queue_depth: u64, pub queue_capacity: u64, pub capture_errors: u64 }
#[derive(Debug)]
...
#[cfg(windows)]
pub struct CaptureSession { stop_tx: Option<Sender<()>>, worker: Option<JoinHandle<()>>, metrics: Arc<CaptureMetricsAtomic>, queue_capacity: usize }
#[cfg(windows)]
...
impl CaptureMetricsAtomic {
    fn snapshot(&self, capacity: usize) -> CaptureMetrics {
        CaptureMetrics { packets_received: self.received.load(Ordering::Relaxed), packets_dropped: self.dropped.load(Ordering::Relaxed), bytes_received: self.bytes.load(Ordering::Relaxed), queue_depth: self.depth.load(Ordering::Relaxed), queue_capacity: capacity as u64, capture_errors: self.errors.load(Ordering::Relaxed) }
    }
}
#[cfg(windows)]
impl CaptureSession {
    pub fn start(device_name: &str, config: CaptureConfig) -> Result<(Self, Receiver<CaptureEvent>)> {
        ...
        let worker = thread::Builder::new().name("netsight-capture".into()).spawn(move || run_capture_loop(&mut capture, stop_rx, event_tx, worker_metrics)).context("failed to spawn capture worker")?;
...
#[cfg(windows)]
fn run_capture_loop(capture: &mut Capture<Active>, stop_rx: Receiver<()>, event_tx: mpsc::SyncSender<CaptureEvent>, metrics: Arc<CaptureMetricsAtomic>) {
    loop {
        ...
        match capture.next_packet() {
            Ok(packet) => {
                ...
                metrics.bytes.fetch_add(packet.header.caplen as u64, Ordering::Relaxed);
                ...
                match event_tx.try_send(event) {
                    Ok(()) => {}
                    Err(mpsc::TrySendError::Full(_)) => { metrics.dropped.fetch_add(1, Ordering::Relaxed); }
